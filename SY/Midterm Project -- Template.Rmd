...
iteration_func <- function(model, train_data, test_data = test.data) {
  ...
  Data <- paste("dat_", c(rep(n.values[1], 3), rep(n.values[2], 3), rep(n.values[2], 3)), "_", rep(c(1, 2, 3), 3), sep = "")
  result <- cbind(result[, 4], cbind(Data, result[,1:3]))
  colnames(result) <- c("Model", "Data", "A", "B", "C")
  return(result)
}
```
...
```{r load_model1}
iteration_func(train_data = train_data_sample, model = model_kNN)
```
...
model_randomForest <- function(train_data, test_data = test.data) {
  t1 <- Sys.time()
  outRf <- randomForest(train_data$x, factor(train_data$y), maxnodes = 1000)
...
```{r load_model2}
iteration_func(model = model_randomForest, train_data = train_data_sample)
```
...
model_svm <- function(train_data, test_data = test.data){
  t1 <- Sys.time()
  outSvm <- svm(train_data$x, factor(train_data$y), kernel="radial", cost = 1)
...
```{r load_model3}
iteration_func(model = model_svm, train_data = train_data_sample)
```
